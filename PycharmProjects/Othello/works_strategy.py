...
class Strategy():
    ...
    def mob_stability_weighted_score(self, board, player):
        ...
        if len(board_pieces)<20:
            ...
            status= "Middle"
            score += len(self.get_valid_moves(board, BLACK)) * 3 #mobility
            #score -= len(self.get_valid_moves(board, WHITE)) * 3
        else:
            status= "Late"
    ...
    def alphabeta_strategy(self, board, player, depth=3):
        ...
        move = self.alphabeta_search(Node(board), player, -1000000, 1000000, depth=4).move
        return move
